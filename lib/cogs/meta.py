...
class Meta(Cog):
	def __init__(self, bot):
		...
		self._message = "watching {users:,} users in {guilds:,} servers"
	...
	async def ping(self, ctx):
		...
		await message.edit(content=f"DWSP Latency: {self.bot.latency*1000:,.0f} ms. Reponse Time:{(end-start)*1000:,.0f} ms.")

	@command(name="stats")
	async def show_bot_stats(self, ctx):
		embed = Embed(title="Bot Statistics",
